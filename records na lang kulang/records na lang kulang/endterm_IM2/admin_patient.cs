...
namespace endterm_IM2
{
    public partial class admin_patient : Form
    {
        ...
        private void stud_datagridView_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            this.stud_emer_no.Text = this.stud_datagridView.SelectedRows[0].Cells[11].Value.ToString();
            this.stud_emer_add.Text = this.stud_datagridView.SelectedRows[0].Cells[12].Value.ToString();
        ...
        private void stud_edit_Click(object sender, EventArgs e)
        {
            ...
            else if (stud_edit.Text == "UPDATE")
            {
                ...
                string sqlUpdate = "UPDATE `students` SET FirstName = '" + this.stud_Fname.Text + "', LastName = '" + this.stud_LName.Text + "', Age = '" + this.stud_age.Text + "', Sex = '" + this.groupBox1.Text + "', DateOfBirth = '" + this.dateTimePicker1.Text + "', Year_Level = '" + this.stud_level.Text + "', Contact_number = '" + this.stud_contact.Text + "', Address='" + this.stud_address.Text + "',program_course='" + this.stud_course.Text + "', Emer_ContactName= '" + this.stud_emer_name.Text + "', Emer_ContactNum = '" + this.stud_emer_no.Text + "', emerContactAddress= '" + this.stud_emer_add.Text + "' WHERE StudentID = '" + this.student_id.Text + "'";
        ...
        private void stud_level_TextChanged(object sender, EventArgs e)
        {
            
        }

        private void stud_age_TextChanged(object sender, EventArgs e)
        {
            if (!System.Text.RegularExpressions.Regex.IsMatch(stud_age.Text, "^[0-9]+$"))
            {
                stud_age.Text = "";
            }
        }

        private void stud_contact_TextChanged(object sender, EventArgs e)
        {
            if (!System.Text.RegularExpressions.Regex.IsMatch(stud_contact.Text, "^[0-9]+$"))
            {
                stud_contact.Text = "";
            }
        }

        private void stud_emer_no_TextChanged(object sender, EventArgs e)
        {
            if (!System.Text.RegularExpressions.Regex.IsMatch(stud_emer_no.Text, "^[0-9]+$"))
            {
                stud_emer_no.Text = "";
            }
        }
        ...
        {
            int rowHeight = stud_datagridView.RowTemplate.Height + 5; // Adjust this value based on your preference
            int cellMargin = 12; // Adjust this value based on your preference
            int fontSize = 12; // Adjust this value based on your preference

            Font titleFont = new Font(stud_datagridView.Font.FontFamily, fontSize + 10, FontStyle.Bold); // Larger title font
            Font headerFont = new Font(stud_datagridView.Font.FontFamily, fontSize + 0, FontStyle.Bold);
            Font cellFont = new Font(stud_datagridView.Font.FontFamily, fontSize);

            int y = e.MarginBounds.Top;

            // Display the title
            e.Graphics.DrawString("Patient's List", titleFont, Brushes.Black, cellMargin, y);
            y += titleFont.Height + 20; // Add some spacing below the title

            string[] columnsToDisplay = { "StudentID","FirstName", "LastName", "Age", "Sex", "Year_Level", "Contact_Number" };

            foreach (string columnName in columnsToDisplay)
            {
                DataGridViewColumn column = stud_datagridView.Columns[columnName];
            ...
            foreach (DataGridViewRow row in stud_datagridView.Rows)
            {
